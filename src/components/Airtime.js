...
export default function Airtime({ props }) {
    ...
    return (
        <div className="airtime fixed z-10 top-0 right-0 h-screen w-full bg-blue-600 bg-opacity-25 flex items-center justify-center" onClick={(e) => close(e)}>
            <div className="tilt-in-fwd-tr p-3 w-1/5 shadow-lg bg-white">
                <form onSubmit={e => Submit(e)}>
                    <div>
                        <label htmlFor="price">Amount</label>
                        <input className="p-3 my-3 bg-blue-500 rounded" type="number" onChange={(e) => airtimeDetails({ ...airtime, amount: e.target.value })} />
                    </div>
                    <div>
                        <label htmlFor="phoneNumber"> Phone Number </label>
                        <input className="p-3 my-3 bg-blue-500 rounded" type="number" onChange={(e) => airtimeDetails({ ...airtime, phoneNumber: e.target.value })} />
                    </div>
